...
output "dns_hostnames" {
  description = "Custom domain hostnames (prod segment)"
...
output "load_balancer_ip" {
  description = "GoDaddy A record target for app / api / admin .littlescout.app"
...
output "budget_alert_name" {
  ...
  value       = var.enable_billing_budget && var.billing_account_id != "" ? module.budget_alert[0].budget_name : null
}
