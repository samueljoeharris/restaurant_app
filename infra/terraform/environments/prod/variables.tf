...
variable "enable_cloud_sql" {
  type        = bool
  description = "Phase B: provision Cloud SQL. Keep false until API deploy."
  default     = false
}
...
variable "enable_restaurant_refresh_job" {
  type        = bool
  description = "Pub/Sub seed queue + weekly Scheduler → API refresh. Requires enable_cloud_run = true."
  default     = false
...
variable "cloud_sql_tier" {
  type        = string
  description = "Cloud SQL machine tier. Prod default is a real tier, not micro."
  default     = "db-custom-1-3840"
}

variable "deletion_protection" {
  type        = bool
  description = "Prevent accidental destroy of stateful prod resources (Cloud SQL)"
  default     = true
}
...
variable "budget_notification_emails" {
  type        = list(string)
  description = "Emails to notify via the budget alert's notification channels"
  default     = []
}
...
variable "enable_admin_iap" {
  type        = bool
  description = "Enable Identity-Aware Proxy on admin load balancer backend"
  default     = true
...
variable "enable_sa_key_max_age_policy" {
  type        = bool
  description = "Enforce max age on service account JSON keys (90d default)."
  default     = true
}
