...
| `.github/workflows/` | CI/CD pipeline (`deploy.yml`), reusable deploy modules (`reusable/`), and manual tools (`tools/`). |
...
| `.github/workflows/tools/debug-logs.yml` | Manual Cloud Logging fetch for ops debugging. |
| `.github/workflows/tools/ios.yml` | Manual iOS build (`workflow_dispatch`). |
