...
Manual iOS build workflow `.github/workflows/tools/ios.yml` (`workflow_dispatch` only today; slot already planned in DESIGN.md §10):
...
| **M1** | Read-only browse + CI | Models, `APIClient`, list + search, restaurant detail with TTF badge against dev API; `tools/ios.yml` build+test | ✅ browse in simulator; CI workflow is manual `workflow_dispatch` until signing secrets exist |
