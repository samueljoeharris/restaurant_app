...
final class PlaceSearchViewModel {
    ...
    private func pollSeedJob(jobId: String, lat: Double, lng: Double) async {
        ...
        for _ in 0 ..< maxAttempts {
            ...
            do {
                status = try await api.coverageJob(id: jobId)
            } catch {
                // Transient network error — keep polling.
                continue
            }

            if status.status == "complete" || status.status == "done" || status.status == "finished" {
