"""Refresh the restaurant catalog (Place Details hygiene pass) from Google Places."""
...
from ttf_api.seed_jobs import create_catalog_refresh_job, get_seed_job, run_seed_job


def main() -> int:
    run_migrations()
    try:
        created = create_catalog_refresh_job()
        if not created:
